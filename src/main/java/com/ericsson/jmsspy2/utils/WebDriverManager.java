...
public class WebDriverManager {
	public static final String DOWNLOAD_DIRECTORY = System
	...
	private static WebDriver getWebDriver(Browser browser) {
		...
		case CHROME:
			/**
			 * need to set system property with path of chromedriver to make it
			 * working https://code.google.com/p/selenium/wiki/ChromeDriver#
			 * Getting_Started
			 */
			String chromePath = "src/main/resources/drivers/chromedriver.exe";
			...
			// String.format("--lang=%s", language));
			return new ChromeDriver(options);

		case FIREFOX:
			/*
			 * https://code.google.com/p/selenium/issues/detail?id=5050 The
			 * issue is that firefox waits for the Dashboard page to load while
			 * there is some jquery which is continuously checks for updates -
			 * this makes firefox unresponsive
			 */
			DesiredCapabilities firefoxCaps = DesiredCapabilities.firefox();
			// List of constants:
			// https://dvcs.w3.org/hg/webdriver/raw-file/tip/webdriver-spec.html#page-load-strategies-1/>
			firefoxCaps.setCapability(CapabilityType.PAGE_LOADING_STRATEGY,
					"none");
			// https://code.google.com/p/selenium/wiki/DesiredCapabilities#Read-write_capabilities
			firefoxCaps.setCapability(CapabilityType.ELEMENT_SCROLL_BEHAVIOR,
					ElementScrollBehavior.BOTTOM);
			FirefoxProfile profile = new FirefoxProfile();
			// profile.setPreference("intl.accept_languages",
			// language.toString());
			profile.setPreference("browser.download.folderList", 2); // 0 -
																		// Desktop,
																		// 1 -
																		// System
																		// default,
		...
		case IE:
			String pathToIEDriver = "src/main/resources/drivers/IEDriverServer.exe";
			...
			caps.setCapability(
					...
					true);
			// This code is commented out since IEDriver v2.43 crashes with this
			// flag set up
			// https://code.google.com/p/selenium/wiki/DesiredCapabilities#Read-write_capabilities
			// caps.setCapability(CapabilityType.ELEMENT_SCROLL_BEHAVIOR,
			// ElementScrollBehavior.BOTTOM);
			WebDriver wd = new InternetExplorerDriver(caps);
			wd.manage().deleteAllCookies();
			return wd;
		default:
			throw new RuntimeException("Please specify valid browser");
		}
	}
	...
	public static WebDriver getDriver() {
		...
		wd.manage().timeouts()
				.pageLoadTimeout(DEFAULT_TIMEOUT_SEC, TimeUnit.SECONDS);
